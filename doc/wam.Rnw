...
The function \emph{wam} provide a high level interface to the actual functions.

The data are best represented in a data frame giving for each form:
\begin{itemize}
\item the form
\item the subcorpus name
\item k: the frequency of the form in the subcorpus
\item N: the number of occurrences in the corpus
\item K: the frequency of the for in the corpus
\item n: the number of occurrences in the subcorpus
\end{itemize}

<<wam_data, echo=TRUE, fig=FALSE>>=
...
The result can be printed, sorted by one of the computed indicator, and filtered to one of the subcorpus under scrutiny:
